...
export default function Toolbar({ onAdd, isMobile = false }: Props) {
  return (
    ...
    <aside
      className={
        isMobile
          ? "flex w-full flex-row items-stretch gap-2 overflow-x-auto px-3 py-2 lg:w-[72px] lg:flex-col lg:items-center lg:overflow-visible lg:border-b-0 lg:border-r lg:px-0 lg:py-4"
          : "px-2 border-r flex flex-col items-center py-4 gap-2"
      }
    >
      <p
        className={
      ...
      {FIXTURE_TYPES.map((type) => (
        <Button
          ...
          className={
            isMobile
              ? "bg-white hover:bg-white h-14 min-w-24 flex flex-col items-center justify-center gap-1 border transition-all group shrink-0 lg:w-12"
              : "bg-white hover:bg-white w-full h-14 flex flex-col items-center justify-center gap-1 border border-current transition-all group"
          }
        >
          {FIXTURE_ICONS[type]}
